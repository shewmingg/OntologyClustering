...
public class DennaiSimilarity extends Similarity {
		...
		for(int i=0;i<context.getSim().size();i++){
			for(int j=i;j<context.getSim().size();j++){
				if(cpidx.get(i).get(j)!=-1){
					if(cpidx.get(i).get(j)==i || cpidx.get(i).get(j)==j){
						...
						context.getSim().get(i).set(j, 2.0*context.getDepth().get(cpidx.get(i).get(j))/(context.getDepth().get(i)+context.getDepth().get(j)+(context.getDepth().get(i)*context.getDepth().get(j))));
						context.getSim().get(j).set(i, 2.0*context.getDepth().get(cpidx.get(i).get(j))/(context.getDepth().get(i)+context.getDepth().get(j)+(context.getDepth().get(i)*context.getDepth().get(j))));
					}
				}
